...
io.on('connection', (socket) => {
    ...
    // Caller announces they're starting a video call in the room
    socket.on('vc-start-call', ({ roomId, callerNickname }) => {
        const user = users[socket.id];
        if (!user || !roomId) return;
        // Notify everyone else in the room about the incoming call
        socket.to(roomId).emit('vc-call-started', {
            callerId: socket.id,
            callerNickname: callerNickname || (user.nickname) || 'Someone'
        });
        // Tell others that caller joined call
        socket.to(roomId).emit('vc-peer-joined', {
            socketId: socket.id,
            nickname: callerNickname || (user.nickname) || 'Someone'
        });
        console.log(`[VC] ${callerNickname} started a call in room ${roomId}`);
    });

    // A user accepted and joined the call
    socket.on('vc-join-call', ({ roomId, nickname }) => {
        if (!roomId) return;
        // Notify everyone else they joined
        socket.to(roomId).emit('vc-peer-joined', {
            socketId: socket.id,
            nickname: nickname || (users[socket.id] && users[socket.id].nickname) || 'Someone'
        });
        console.log(`[VC] ${nickname} joined call in room ${roomId}`);
    });

    // Someone declined the call
    socket.on('vc-decline-call', ({ roomId }) => {
        if (roomId) socket.to(roomId).emit('vc-call-declined', { socketId: socket.id });
    });

    // Caller ended the call for everyone
    socket.on('vc-end-call', ({ roomId }) => {
        if (roomId) {
            socket.to(roomId).emit('vc-call-ended');
            socket.to(roomId).emit('vc-peer-left', { socketId: socket.id });
        }
        console.log(`[VC] Call ended in room ${roomId} by ${socket.id}`);
    });

    // WebRTC Offer (caller → specific peer)
    socket.on('vc-offer', ({ targetId, offer, roomId }) => {
        if (targetId) {
            io.to(targetId).emit('vc-offer', {
                senderId: socket.id,
                offer,
                roomId
            });
    ...
    // WebRTC Answer (callee → specific caller)
    socket.on('vc-answer', ({ targetId, answer, roomId }) => {
        if (targetId) {
            io.to(targetId).emit('vc-answer', {
                senderId: socket.id,
                answer,
                roomId
            });
    ...
    // ICE Candidate relay
    socket.on('vc-ice', ({ targetId, candidate, roomId }) => {
        if (targetId) {
            io.to(targetId).emit('vc-ice', {
                senderId: socket.id,
                candidate,
                roomId
            });
    ...
    socket.on('vc-media-state', ({ roomId, micMuted, camOff }) => {
        if (roomId) {
            socket.to(roomId).emit('vc-media-state', { socketId: socket.id, micMuted, camOff });
        }
    });
