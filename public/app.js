...
async function runFaceScanLoop() {
    ...
    try {
        ...
        if (!detection) {
            ...
            if (faceScanDetailEl) faceScanDetailEl.textContent = 'Position face inside the green guide';
            return;
        }
...
// ═══════════════════════════════════════════════════════════════
// VIDEO CALL SYSTEM — WebRTC + Socket.IO Signaling
// ═══════════════════════════════════════════════════════════════

const VC = (() => {
    // DOM references
    let _screen, _micBtn, _camBtn, _endBtn, _shareBtn, _chatBtn, _participantsBtn;
    let _localVideo, _remoteVideo, _timer, _roomLabel, _peopleCount;
    let _selfView, _selfNoCam, _remoteAvatar, _remoteName, _remoteMicIndicator;
    let _participantsPanel, _participantsList, _closePanelBtn;
    let _incomingOverlay, _incomingCallerName, _acceptBtn, _declineBtn;
    let _headerCallBtn;
    let _gridEl;

    // State
    let _localStream = null;
    let _peerConnections = {}; // { socketId: RTCPeerConnection }
    let _micMuted = false;
    let _camOff = false;
    let _sharing = false;
    let _inCall = false;
    ...
    let _timerSecs = 0;
    let _participants = {}; // { socketId: nickname }
    let _myNickname = 'You';
    ...
    function init() {
        _screen            = _dom('video-call-screen');
        _micBtn            = _dom('vc-mic-btn');
        _camBtn            = _dom('vc-cam-btn');
        _endBtn            = _dom('vc-end-btn');
        _shareBtn          = _dom('vc-share-btn');
        _chatBtn           = _dom('vc-chat-btn');
        _participantsBtn   = _dom('vc-participants-btn');
        _localVideo        = _dom('vc-local-video');
        _remoteVideo       = _dom('vc-remote-video');
        _timer             = _dom('vc-timer');
        _roomLabel         = _dom('vc-room-label');
        _peopleCount       = _dom('vc-people-count');
        _selfView          = _dom('vc-self-view');
        _selfNoCam         = _dom('vc-self-no-cam');
        _remoteAvatar      = _dom('vc-remote-avatar');
        _remoteName        = _dom('vc-remote-name');
        _remoteMicIndicator = _dom('vc-remote-mic-indicator');
        _participantsPanel = _dom('vc-participants-panel');
        _participantsList  = _dom('vc-participants-list');
        _closePanelBtn     = _dom('vc-close-panel-btn');
        _incomingOverlay   = _dom('vc-incoming-overlay');
        _incomingCallerName = _dom('vc-incoming-caller-name');
        _acceptBtn         = _dom('vc-accept-btn');
        _declineBtn        = _dom('vc-decline-btn');
        _headerCallBtn     = _dom('video-call-btn');
        _gridEl            = _dom('vc-grid');

        _bindControls();
        _bindSocketEvents();
        _makeSelfViewDraggable();
    }

    function _bindControls() {
        if (_headerCallBtn) {
            addFastClickListener(_headerCallBtn, () => {
                if (_inCall) { _endCall(); }
                else { startCall(); }
            });
        }
        if (_micBtn)          addFastClickListener(_micBtn,          toggleMic);
        if (_camBtn)          addFastClickListener(_camBtn,          toggleCam);
        if (_endBtn)          addFastClickListener(_endBtn,          _endCall);
        if (_shareBtn)        addFastClickListener(_shareBtn,        toggleScreenShare);
        if (_chatBtn)         addFastClickListener(_chatBtn,         _goToChat);
        if (_participantsBtn) addFastClickListener(_participantsBtn, _toggleParticipantsPanel);
        if (_closePanelBtn)   addFastClickListener(_closePanelBtn,   () => _participantsPanel && _participantsPanel.classList.add('hidden'));
        if (_acceptBtn)       addFastClickListener(_acceptBtn,       _acceptCall);
        if (_declineBtn)      addFastClickListener(_declineBtn,      _declineCall);
        if (_dom('vc-grid-btn')) addFastClickListener(_dom('vc-grid-btn'), _toggleGridLayout);
    }

    function _bindSocketEvents() {
        if (!window.socket) {
            // Retry after socket is ready
            setTimeout(_bindSocketEvents, 800);
            return;
        }
        const s = window.socket;

        // Someone in the room started a call
        s.on('vc-call-started', (data) => {
            if (_inCall) return; // already in call
            _pendingCallerId = data.callerId;
            if (_incomingCallerName) _incomingCallerName.textContent = data.callerNickname || 'Someone';
            if (_incomingOverlay) _incomingOverlay.classList.remove('hidden');
            // Play ring sound if available
            try { if (typeof SoundManager !== 'undefined') SoundManager.play('msg'); } catch(e) {}
        });

        // WebRTC signals
        s.on('vc-offer',     (data) => _handleOffer(data));
        s.on('vc-answer',    (data) => _handleAnswer(data));
        s.on('vc-ice',       (data) => _handleIce(data));

        // Someone else's mic/cam state
        s.on('vc-media-state', (data) => {
            if (data.micMuted && _remoteMicIndicator) _remoteMicIndicator.classList.remove('hidden');
            else if (_remoteMicIndicator) _remoteMicIndicator.classList.add('hidden');
        });

        // Participant joined call
        s.on('vc-peer-joined', (data) => {
            _participants[data.socketId] = data.nickname;
            _updateParticipantsUI();
            // Create peer connection for this new peer (we're the existing user)
            if (_inCall) {
                _createPeerConnection(data.socketId, true);
            }
        });

        // Someone left call
        s.on('vc-peer-left', (data) => {
            _cleanupPeer(data.socketId);
            delete _participants[data.socketId];
            _updateParticipantsUI();
            // Show waiting state if no peers
            if (Object.keys(_peerConnections).length === 0) {
                if (_remoteVideo) _remoteVideo.srcObject = null;
                if (_remoteAvatar) _remoteAvatar.classList.remove('hidden');
            }
        });
    ...
    async function startCall() {
        ...
        if (!roomId) {
            _showToast('Join a room first to start a video call.', 'warn');
            return;
        }
        try {
            ...
            // Try audio only
            try { _localStream = await navigator.mediaDevices.getUserMedia({ video: false, audio: true }); _camOff = true; }
            catch(e2) { _showToast('Camera/microphone not available.', 'error'); return; }
        }
        _myNickname = (typeof myNickname !== 'undefined') ? myNickname : 'You';
        _callRoomId = roomId;
        _inCall = true;
        _openScreen();
        _attachLocalStream();
        _participants[window.socket.id] = _myNickname;
        _updateParticipantsUI();
        // Notify room
        window.socket.emit('vc-start-call', {
            roomId: roomId,
            callerNickname: _myNickname
        });
        _startTimer();
        if (_headerCallBtn) _headerCallBtn.classList.add('call-active');
        if (_roomLabel) _roomLabel.textContent = roomId;
    }

    // ── Accept incoming call ──────────────────────────────────────
    async function _acceptCall() {
        if (_incomingOverlay) _incomingOverlay.classList.add('hidden');
        const roomId = (typeof currentRoomID !== 'undefined') ? currentRoomID : null;
        if (!roomId) return;
        try {
            _localStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
        ...
        _participants[window.socket.id] = _myNickname;
        _updateParticipantsUI();
        window.socket.emit('vc-join-call', { roomId: roomId, nickname: _myNickname });
        _startTimer();
        if (_headerCallBtn) _headerCallBtn.classList.add('call-active');
        if (_roomLabel) _roomLabel.textContent = roomId;
    }
    ...
    function _declineCall() {
        ...
        window.socket && window.socket.emit('vc-decline-call', { roomId: currentRoomID });
    }

    // ── End call ─────────────────────────────────────────────────
    function _endCall(silent) {
        _inCall = false;
        _stopTimer();
        if (!silent && _callRoomId && window.socket) {
            window.socket.emit('vc-end-call', { roomId: _callRoomId });
        }
        // Cleanup all peers
        Object.keys(_peerConnections).forEach(id => _cleanupPeer(id));
        _peerConnections = {};
        _participants = {};
        // Stop local tracks
        if (_localStream) {
        ...
        if (_remoteVideo) _remoteVideo.srcObject = null;
        // Reset state
        _micMuted = false; _camOff = false; _sharing = false;
        _closeScreen();
        if (_headerCallBtn) _headerCallBtn.classList.remove('call-active');
        _callRoomId = null;
        _timerSecs = 0;
        if (_timer) _timer.textContent = '00:00';
    }
    ...
    function _openScreen() {
        ...
        _screen.classList.add('vc-enter');
        setTimeout(() => _screen && _screen.classList.remove('vc-enter'), 400);
        // Update button states
        _updateMicBtn(); _updateCamBtn();
        // Reset waiting state
        if (_remoteAvatar) _remoteAvatar.classList.remove('hidden');
        if (_remoteVideo) _remoteVideo.srcObject = null;
    }
    ...
    function _closeScreen() {
        ...
        if (_participantsPanel) _participantsPanel.classList.add('hidden');
        if (_incomingOverlay) _incomingOverlay.classList.add('hidden');
    }

    function _goToChat() {
        // Minimize call screen (keep call alive, close screen)
        _screen && _screen.classList.add('hidden');
        _showToast('Call minimized. Click 📹 to return.', 'info');
        // Allow reopening by clicking header btn
        if (_headerCallBtn) _headerCallBtn.onclick = () => {
            _screen && _screen.classList.remove('hidden');
            _headerCallBtn.onclick = null;
        };
    }
    ...
    function _startTimer() {
        ...
        _timerInterval = setInterval(() => {
            ...
            if (_timer) _timer.textContent = `${m}:${s}`;
        }, 1000);
    }
    function _stopTimer() {
    ...
    function toggleMic() {
        ...
        _updateMicBtn();
        window.socket && window.socket.emit('vc-media-state', { roomId: _callRoomId, micMuted: _micMuted, camOff: _camOff });
    }
    ...
    function toggleCam() {
        ...
        _updateCamBtn();
        if (_selfNoCam) { if (_camOff) _selfNoCam.classList.remove('hidden'); else _selfNoCam.classList.add('hidden'); }
        window.socket && window.socket.emit('vc-media-state', { roomId: _callRoomId, micMuted: _micMuted, camOff: _camOff });
    }
    ...
    async function toggleScreenShare() {
        if (!_sharing) {
            try {
                ...
                Object.values(_peerConnections).forEach(pc => {
                    const sender = pc.getSenders().find(s => s.track && s.track.kind === 'video');
                    if (sender) sender.replaceTrack(screenTrack);
                });
                if (_localVideo) _localVideo.srcObject = screenStream;
                _sharing = true;
                if (_shareBtn) { _shareBtn.classList.add('off'); _shareBtn.querySelector('span').textContent = 'Stop'; }
                screenTrack.onended = () => { _sharing = false; _stopScreenShare(); };
    ...
    function _stopScreenShare() {
        _sharing = false;
        if (_shareBtn) { _shareBtn.classList.remove('off'); _shareBtn.querySelector('span').textContent = 'Share'; }
        // Restore camera
        if (_localStream) {
            ...
            if (camTrack) {
                ...
                if (_localVideo) _localVideo.srcObject = _localStream;
            }
        }
    }
    ...
    function _updateMicBtn() {
        if (!_micBtn) return;
        const icon = _micBtn.querySelector('i');
        const label = _micBtn.querySelector('span');
        if (_micMuted) {
            _micBtn.classList.add('off');
            if (icon) icon.className = 'fas fa-microphone-slash';
            if (label) label.textContent = 'Unmute';
        } else {
            _micBtn.classList.remove('off');
    ...
    function _updateCamBtn() {
        if (!_camBtn) return;
        const icon = _camBtn.querySelector('i');
        const label = _camBtn.querySelector('span');
        if (_camOff) {
            _camBtn.classList.add('off');
            if (icon) icon.className = 'fas fa-video-slash';
            if (label) label.textContent = 'Start';
        } else {
            _camBtn.classList.remove('off');
    ...
    async function _createPeerConnection(remoteSocketId, isInitiator) {
        if (_peerConnections[remoteSocketId]) return _peerConnections[remoteSocketId];
        ...
        pc.ontrack = (e) => {
            if (_remoteVideo) {
                ...
                if (_remoteAvatar) _remoteAvatar.classList.add('hidden');
            }
        };
        ...
        pc.onconnectionstatechange = () => {
            if (pc.connectionState === 'disconnected' || pc.connectionState === 'failed' || pc.connectionState === 'closed') {
                _cleanupPeer(remoteSocketId);
                delete _participants[remoteSocketId];
                _updateParticipantsUI();
            }
        };
        ...
        if (isInitiator) {
            ...
            window.socket && window.socket.emit('vc-offer', {
                targetId: remoteSocketId,
                offer: offer,
                roomId: _callRoomId
            });
    ...
    async function _handleOffer(data) {
        const pc = await _createPeerConnection(data.senderId, false);
        await pc.setRemoteDescription(new RTCSessionDescription(data.offer));
        const answer = await pc.createAnswer();
        await pc.setLocalDescription(answer);
        window.socket && window.socket.emit('vc-answer', {
    ...
    async function _handleAnswer(data) {
        const pc = _peerConnections[data.senderId];
        if (pc) await pc.setRemoteDescription(new RTCSessionDescription(data.answer));
    }

    async function _handleIce(data) {
        const pc = _peerConnections[data.senderId];
    ...
    function _cleanupPeer(socketId) {
        const pc = _peerConnections[socketId];
        if (pc) { try { pc.close(); } catch(e) {} delete _peerConnections[socketId]; }
    }

    // ── Participants UI ───────────────────────────────────────────
    function _updateParticipantsUI() {
        const count = Object.keys(_participants).length;
        if (_peopleCount) _peopleCount.textContent = count;
        if (!_participantsList) return;
        _participantsList.innerHTML = Object.entries(_participants).map(([sid, name]) => {
            const isMe = sid === (window.socket && window.socket.id);
            const letter = (name || 'A')[0].toUpperCase();
            return `<div class="vc-participant-row">
                <div class="vc-participant-dot">${letter}</div>
                <div class="vc-participant-name">${_esc(name)} ${isMe ? '<span class="vc-participant-you">(You)</span>' : ''}</div>
                <div class="vc-participant-icons"><i class="fas fa-microphone"></i><i class="fas fa-video" style="margin-left:4px;"></i></div>
            </div>`;
        }).join('');
    }

    function _toggleParticipantsPanel() {
        if (!_participantsPanel) return;
        _participantsPanel.classList.toggle('hidden');
        _updateParticipantsUI();
    }

    // ── Grid layout toggle ────────────────────────────────────────
    function _toggleGridLayout() {
        if (!_gridEl) return;
        const has2 = _gridEl.classList.contains('grid-2');
        _gridEl.className = 'vc-grid' + (has2 ? '' : ' grid-2');
    }

    // ── Draggable self-view ───────────────────────────────────────
    function _makeSelfViewDraggable() {
        if (!_selfView) return;
        let dragging = false, startX, startY, origLeft, origBottom;
        _selfView.addEventListener('mousedown', (e) => {
            dragging = true;
            startX = e.clientX; startY = e.clientY;
            const rect = _selfView.getBoundingClientRect();
            origLeft = rect.left;
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            _selfView.style.left = Math.max(0, origLeft + dx) + 'px';
            _selfView.style.bottom = Math.max(0, origBottom - dy) + 'px';
            _selfView.style.right = 'auto';
        });
        document.addEventListener('mouseup', () => { dragging = false; });

        // Touch
        _selfView.addEventListener('touchstart', (e) => {
            const t = e.touches[0];
            dragging = true;
    ...
    function _showToast(msg, type) {
        ...
        t.style.cssText = 'position:fixed;bottom:100px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.8);color:white;padding:10px 18px;border-radius:12px;font-size:0.85rem;z-index:99999;pointer-events:none;';
        t.textContent = msg;
    ...
    // Public API
    return { init, startCall, toggleMic, toggleCam, toggleScreenShare };
})();
